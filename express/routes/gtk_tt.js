...
function tick_gtk_tt(data){
    ...
    return {
        ...
        file_name: moment(now).format("YYYY_MM"),
        time_block: `${moment(now).format('YYYY MM')} `+
        `${moment(now).format('MMMM').charAt(0).toUpperCase()}`+
...
router.get('/', function(req, res) {
    ...
    {
        title: moment().add(req.query.m, "month").format("MM-YYYY"),
